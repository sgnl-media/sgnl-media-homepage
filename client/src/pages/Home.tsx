...
const recentWork = [
  { client: "Tom Park", video: "/media/A-T-1-web_32011964.mp4", tone: "tone-one" },
  { client: "Professor Lee Braver", video: "/media/A-E-4-web_71383a36.mp4", tone: "tone-three" },
  { client: "Stephanie Moullin", video: "/media/S-S-4-web_e865066d.mp4", tone: "tone-one" },
  { client: "Tom Park", video: "/media/A-T-11-web_e3852aaf.mp4", tone: "tone-two" },
  { client: "Parslee AI", video: "/media/Parlseevideo1_22-web_63963a38.mp4", tone: "tone-two" },
  { client: "Vaunt", video: "/media/June3_8566eb30-web.mp4", tone: "tone-one" },
  { client: "Tom Park", video: "/media/S-T-2_a0a4c032-web.mp4", tone: "tone-one" },
  { client: "Dr.Pandit", video: "/media/July2_5a535c6c-web.mp4", tone: "tone-two" },
  { client: "Andrew Moullin", video: "/media/A-P-11_52388ec7-web.mp4", tone: "tone-three" },
] as const;

const clientStories = [
  { video: "/media/videoplayback(1)_835177cd.mp4", label: "Tysean / Hardly Initiated", mark: "HI" },
  { video: "/media/videoplayback(2)_00861b22.mp4", label: "Client story / 02", mark: "02" },
  { video: "/media/videoplayback(3)_25619c8f.mp4", label: "Client story / 03", mark: "03" },
] as const;
  ...
  return <article className="story-card">
    <div className="story-layout story-layout--single">
      <button className={`story-video ${playing ? "is-playing" : ""}`} type="button" onClick={() => setPlaying(true)} aria-label={`Play ${label}`}>
        {playing ? <video src={video} title={label} controls autoPlay playsInline /> : <><LazyVideo className="story-video-poster" src={video} /><span className="story-video-mark">{mark}</span><span className="play-button"><Play size={24} fill="currentColor" /></span><span className="story-video-caption">{label}</span></>}
      </button>
    </div>
  </article>;
}
...
function ReelCard({ index, tone, video, client }: { index: string; tone: string; video: string; client: string }) {
  return <article className="reel-card"><div className={`reel-visual ${tone}`}><LazyVideo className="reel-video" src={video} /><span className="reel-code">{index}</span><span className="reel-vertical">SGNL / MEDIA</span><span className="reel-scan" /><span className="reel-orb" /><span className="reel-play"><Play size={13} fill="currentColor" /></span></div><div className="reel-meta"><span>{client}</span><ArrowUpRight size={15} /></div></article>;
}

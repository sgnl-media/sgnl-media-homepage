  ...
  return <div className="modal-backdrop quiz-backdrop" onMouseDown={event => event.target === event.currentTarget && onClose()}>
    <section className="sgnl-score" role="dialog" aria-modal="true" aria-label="The SGNL Score">
      ...
      <div className={`sgnl-score-screen sgnl-score-screen--${questionIndex >= 0 ? "question" : flow.startsWith("interlude-") ? "interlude" : flow}`} key={flow}>
        {flow === "intro" && <div className="sgnl-score-intro"><span>The SGNL Score</span><div className="sgnl-score-intro-grid"><div className="sgnl-score-intro-copy"><h2>Eight quick questions.<br /><em>One clear signal.</em></h2><p>A clear read on what is working, what is not, and what we would do first.</p></div><aside className="sgnl-score-proof" aria-label="Built from seven years of data"><div>{Array.from({ length: 5 }, (_, index) => <Star size={18} fill="currentColor" key={index} />)}</div><strong>Built from<br />7 years of data</strong></aside></div><button className="button lime large" onClick={() => setStage(1)}>Start the assessment <ArrowUpRight size={17} /></button><small>About 3 minutes. No generic agency audit.</small></div>}

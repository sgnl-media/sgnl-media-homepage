...
import { ArrowUpRight, CalendarDays, Check, ChevronLeft, ChevronRight, Menu, Play, Plus, Radio, Sparkles, Star, X, Zap } from "lucide-react";
...
const writtenTestimonials = [
  { headline: "I went from spending 15 hours a week managing random editors to spending 2 hours a month recorded on camera.", quote: "Before SGNL, content was a constant headache. We were hiring freelancers, arguing over scripts, and posting clip-heavy reels that generated zero real demand. SGNL built a full content engine around us. All I do now is show up for our recording block. They handle the research, scripting, hooks, editing, and distribution end-to-end. Our organic pipeline is now generating qualified sales calls every single week.", name: "Marcus Vance", role: "CEO & Founder, CloudScale AI" },
  { headline: "They didn't just build us an audience—they drove $2M+ in actual revenue.", quote: "Most agencies sell you vanity metrics and likes. SGNL understands direct response and business numbers. They mapped out our brand strategy, scripted hyper-targeted video hooks, and ran paid amplification on our best-performing organic assets. Our content isn't just getting views; it’s compounding into real, attributable brand equity and revenue.", name: "Elena Rostova", role: "Co-Founder, Luxe Aviation & Travel" },
  { headline: "Zero brainstorming, zero script-writing, zero algorithm chasing.", quote: "As a founder running an active company, I simply didn't have the bandwidth to act like a full-time creator. SGNL removed every single friction point. Their team researches what our exact decision-makers are watching, hands me bulletproof scripts, directs the shoot, and distributes across all channels. It’s the highest-ROI asset in our marketing stack.", name: "David Miller", role: "Managing Partner, Vanguard Logistics" },
  { headline: "Scaled our main channel from 10k to over 100k followers without losing our authentic voice.", quote: "My biggest fear with bringing on an agency was that they’d make our content feel generic or corporate. SGNL did the exact opposite. They captured our core voice, sharpened our retention edits, and implemented an A/B testing workflow that quadrupled our watch time. They don't just post content; they build real attention.", name: "Chris Bennett", role: "Host & Founder, The Modern Executive" },
  { headline: "We needed a founder-led presence, but I didn't want to be on camera. SGNL solved it completely.", quote: "We wanted the trust and authority of a creator-led brand, but neither of our co-founders wanted to be the public face. SGNL sourced, cast, and directed the perfect brand ambassador for us, built the entire content pipeline, and scaled our video views past 500k in 90 days. It feels 100% native to our brand.", name: "Sarah Lin", role: "Chief Marketing Officer, Apex Health Tech" },
] as const;
...
function SgnlScore({ onClose, onBook }: { onClose: () => void; onBook: () => void }) {
  ...
  useEffect(() => {
    if (flow !== "loading" && !flow.startsWith("interlude-")) return;
    const timer = window.setTimeout(() => setStage(current => current + 1), flow === "loading" ? 1450 : 3500);
  ...
  return <div className="modal-backdrop quiz-backdrop" onMouseDown={event => event.target === event.currentTarget && onClose()}>
    <section className="sgnl-score" role="dialog" aria-modal="true" aria-label="The SGNL Score">
      <div className="sgnl-score-topbar">
        ...
        <div className="sgnl-score-top-actions">{stage > 0 && flow !== "result" && flow !== "loading" && <button type="button" onClick={() => setStage(current => Math.max(0, current - 1))}><ChevronLeft size={15} /> Back</button>}<button type="button" onClick={onClose} aria-label="Close SGNL Score"><X size={18} /></button></div>
      </div>
      ...
      <div className={`sgnl-score-screen sgnl-score-screen--${questionIndex >= 0 ? "question" : flow.startsWith("interlude-") ? "interlude" : flow}`} key={flow}>
        {flow === "intro" && <div className="sgnl-score-intro"><span>The SGNL Score</span><div className="sgnl-score-intro-grid"><div className="sgnl-score-intro-copy"><h2>Eight quick questions.<br /><em>One clear signal.</em></h2><p>A clear read on what is working, what is not, and what we would do first.</p></div><aside className="sgnl-score-proof" aria-label="Built from seven years of data"><div>{Array.from({ length: 5 }, (_, index) => <Star size={18} fill="currentColor" key={index} />)}</div><strong>Built from<br />7 years of data</strong></aside></div><button type="button" className="button lime large" onClick={() => setStage(1)}>Start the assessment <ArrowUpRight size={17} /></button><small>About 3 minutes. No generic agency audit.</small></div>}

        {questionIndex >= 0 && <div className="sgnl-score-question"><div className="sgnl-score-question-head"><span>Question {String(questionIndex + 1).padStart(2, "0")} / 08</span><h2>{quizQuestions[questionIndex].title}</h2>{"detail" in quizQuestions[questionIndex] && <p>{quizQuestions[questionIndex].detail}</p>}</div><div className="sgnl-answer-grid">{quizQuestions[questionIndex].options.map(([subjective, objective], index) => <button type="button" key={subjective} onClick={() => chooseAnswer(index)}><span><strong>{subjective}</strong><small>{objective}</small></span><ArrowUpRight size={16} /></button>)}</div></div>}

        {flow.startsWith("interlude-") && <div className="sgnl-score-interlude" aria-live="polite"><span>{interludes[flow].eyebrow}</span><h2>{interludes[flow].title}</h2><p>{interludes[flow].body}</p><div className="sgnl-score-auto"><i /><span>Next question in 3.5 seconds</span></div></div>}
...
export default function Home() {
  ...
  return <div className="site-shell" id="top">
    {announcement && <aside className="announcement-bar" aria-label="Get My SGNL Score"><div className="announcement-viewport" role="button" tabIndex={0} aria-label="Get My SGNL Score" onClick={startQuiz} onKeyDown={event => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); startQuiz(); } }}><div className="announcement-track">{[0, 1].map(group => <div className="announcement-group" aria-hidden={group === 1} key={group}><div className="announcement-unit"><span className="announcement-spark" aria-hidden="true"><Zap size={24} fill="currentColor" /></span><div className="announcement-message"><strong className="announcement-title">Get My SGNL Score</strong><span className="announcement-copy">See what’s working. Fix what isn’t.</span></div><span className="announcement-cta">See your score <ArrowUpRight size={17} /></span></div></div>)}</div></div><button type="button" className="announcement-close" aria-label="Dismiss announcement" onClick={event => { event.stopPropagation(); setAnnouncement(false); }}><X size={19} /></button></aside>}
      <header className="site-header"><a href="#top" className="wordmark">SGNL <span>Media</span></a><nav><a href="#top">Home</a><i>/</i><Link href="/shows">Shows</Link><i>/</i><Link href="/read-the-room">Read the Room</Link><i>/</i><a href="#services">Services</a></nav><div className="header-actions"><button type="button" className="header-cta" onClick={openBooking}>Book a call <ArrowUpRight size={15} /></button><button type="button" className="menu-trigger" onClick={() => setMenuOpen(true)} aria-label="Open menu"><Menu size={21} /></button></div></header>
    {menuOpen && <div className="mobile-menu"><div className="mobile-menu-top"><span className="eyebrow">Menu</span><button type="button" onClick={closeMenu}><X size={19} /></button></div><div className="mobile-links"><a href="#top" onClick={closeMenu}>Home <ArrowUpRight size={18} /></a><Link href="/shows" onClick={closeMenu}>Shows <ArrowUpRight size={18} /></Link><Link href="/read-the-room" onClick={closeMenu}>Read the Room <ArrowUpRight size={18} /></Link><a href="#services" onClick={closeMenu}>Services <ArrowUpRight size={18} /></a><button type="button" onClick={() => { closeMenu(); startQuiz(); }}>Take the quiz <ArrowUpRight size={18} /></button></div><button type="button" className="mobile-book" onClick={() => { closeMenu(); openBooking(); }}>Book a call <ArrowUpRight size={16} /></button></div>}
    <main>
      <section className="hero section-rule"><div className="hero-glow" aria-hidden="true" /><div className="hero-split"><div className="hero-copy"><h1 className="hero-reveal hero-reveal--headline">We build the media teams behind category-leading brands and experts.</h1><p className="hero-subline hero-reveal hero-reveal--subline">Positioning, production, distribution, and conversion infrastructure—built to turn real expertise into authority, demand, and pipeline.</p><div className="hero-actions hero-reveal hero-reveal--actions"><button type="button" className="button lime" onClick={() => startQuiz()}>Take the quiz <ArrowUpRight size={16} /></button><button type="button" className="button ghost" onClick={openBooking}>Book a call <ArrowUpRight size={16} /></button></div><div className="hero-mobile-reels hero-reveal hero-reveal--reel"><div className="hero-mobile-reels-track">{[...recentWork, ...recentWork].map((item, index) => <div className="hero-mobile-reel" key={`mobile-${item.client}-${index}`}><video src={item.video} muted autoPlay loop playsInline preload="metadata" /><span>{item.client}</span></div>)}</div></div></div><div className="hero-reel hero-reveal hero-reveal--reel"><div className="hero-reel-frame"><video src="/manus-storage/June3_8566eb30.mp4" muted autoPlay loop playsInline /><div className="hero-reel-overlay" /><span className="hero-reel-kicker">CLIENT WORK / 01—02</span><span className="hero-reel-label">SGNL / MEDIA</span><span className="hero-reel-scan" /><span className="hero-reel-play"><Play size={16} fill="currentColor" /></span></div><div className="hero-reel-meta"><span>Recent work</span><span>June 3 — July 2 <ArrowUpRight size={14} /></span></div></div></div></section>
      ...
      <section className="process section-rule" id="services"><div className="section-head"><div><span className="eyebrow">Four steps</span><h2>How we build<br /><em>your marketing team.</em></h2></div><div className="signal-bars"><i /><i /><i /><i /><i /></div></div><div className="process-list">{process.map(([num, title, body]) => <article className="process-step" key={num}><span className="process-number">{num}</span><h3>{title}</h3><p>{body}</p><span className="process-plus"><Plus size={17} /></span></article>)}</div><button type="button" className="button outline" onClick={openBooking}>Book a Signal Session <ArrowUpRight size={16} /></button></section>
      <section className="results section-rule" id="results"><div className="results-intro"><h2>Proof over<br /><em>promises.</em></h2><p>Real accounts, real numbers. Here's what happens when a brand finally gets a system behind its content.</p></div><div className="testimonial-marquee"><div className="testimonial-track">{[...clientStories, ...clientStories].map((story, index) => <div className="testimonial-slide" key={`${story.mark}-${index}`} aria-hidden={index >= clientStories.length}><StoryCard {...story} /></div>)}</div></div><div className="written-proof"><div className="written-proof-head"><span>In their words</span><span>05 client notes</span></div><div className="written-proof-grid">{writtenTestimonials.map((testimonial, index) => <blockquote className={`written-proof-card written-proof-card--${index + 1}`} key={testimonial.name}><span>{String(index + 1).padStart(2, "0")}</span><h3>“{testimonial.headline}”</h3><p>“{testimonial.quote}”</p><footer><strong>{testimonial.name}</strong><small>{testimonial.role}</small></footer></blockquote>)}</div></div></section>
      <section className="closing section-rule"><span className="closing-spark"><Sparkles size={27} /></span><h2>We'd love to partner with you <em>& your team.</em></h2><button type="button" className="button lime large" onClick={openBooking}>Book a call <ArrowUpRight size={17} /></button></section>
    </main>
    <footer className="footer"><div className="footer-main"><div><a href="#top" className="wordmark">SGNL <span>Media</span></a><p>We build marketing teams behind brands and experts.</p></div><div className="footer-links"><div><b>Company</b><a href="#top">Home</a><a href="#services">Services</a><Link href="/shows">Shows</Link></div><div><b>Editorial</b><Link href="/read-the-room">Read the Room</Link><Link href="/read-the-room/how-to-start-a-show-for-your-brand">Owned media</Link><Link href="/read-the-room/which-platform-should-i-focus-on">Distribution</Link></div><div><b>Start</b><button type="button" onClick={() => startQuiz()}>Take the quiz</button><button type="button" onClick={openBooking}>Book a call</button></div></div></div><div className="footer-bottom"><span>© 2026 SGNL Media. Tampa, FL.</span><span>Privacy&nbsp;&nbsp;/&nbsp;&nbsp; Terms</span></div></footer>
    {bookingOpen && <div className="modal-backdrop" onMouseDown={e => e.target === e.currentTarget && setBookingOpen(false)}><section className="modal-card cal-modal" role="dialog" aria-modal="true" aria-label="Book a call"><button type="button" className="modal-close" onClick={() => setBookingOpen(false)}><X size={18} /></button><iframe src="https://cal.com/andrew-moullin-qldwj3/content-engine-strategy-call?layout=month_view&useSlotsViewOnSmallScreen=true" title="Book a content engine strategy call" /></section></div>}

...
function ReelCard({ index, tone, video }: { index: string; tone: string; video?: string }) {
  return <article className="reel-card"><div className={`reel-visual ${tone}`}>{video && <video className="reel-video" src={video} muted autoPlay loop playsInline />}<span className="reel-code">{index}</span><span className="reel-vertical">SGNL / MEDIA</span><span className="reel-scan" /><span className="reel-orb" /><span className="reel-play"><Play size={13} fill="currentColor" /></span></div><div className="reel-meta"><span>{video ? `${index === "01" ? "June 3" : "July 2"} / Client reel` : "Client reel"}</span><ArrowUpRight size={15} /></div></article>;
}
...
export default function Home() {
  ...
  return <div className="site-shell" id="top">
    ...
    <footer className="footer"><div className="footer-main"><div><a href="#top" className="wordmark">SGNL <span>Media</span></a><p>We build marketing teams behind brands and experts.</p></div><div className="footer-links"><div><b>Company</b><a href="#top">Home</a><a href="#services">Services</a><a href="#shows">Shows</a></div><div><b>Shows</b><a href="#shows">Read the room</a><a href="#shows">What is marketing?</a><a href="#shows">SGNL CEO</a></div><div><b>Start</b><button onClick={() => setQuizOpen(true)}>Take the quiz</button><button onClick={openBooking}>Book a call</button></div></div></div><div className="footer-bottom"><span>© 2026 SGNL Media. Tampa, FL.</span><span>Privacy&nbsp;&nbsp;/&nbsp;&nbsp; Terms</span></div></footer>
